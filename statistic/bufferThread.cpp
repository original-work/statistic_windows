...
int bufferThread::init(String rtsp_addr, deque<Mat> videoFrame)
{
	...
	printf("startTime: %u/%02u/%02u %02u:%02u:%02u\n",
...
void bufferThread::loop_process()
{
	...
	if (!frame.empty()){
		if (capState_ == false) {
			capState_ = true;
			tools::printWithTime("Reinitialized success! NetCamera is working normally!");
		}
		...
		if (capFailCounter_ > 1000) {
			capState_ = false;
			tools::printWithTime("NetCamera fail! Reinitialize it!");
